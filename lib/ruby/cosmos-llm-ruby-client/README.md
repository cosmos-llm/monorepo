...
Cosmos-LLM is a Ruby gem providing a unified interface for interacting with multiple Large Language Model APIs. It simplifies the integration of AI capabilities into Ruby applications by offering a consistent way to access various LLM providers.
...
client = Cosmos::Llm.new(:openai, api_key: 'your-api-key', model: 'gpt-4')
...
client = Cosmos::Llm.new(:openai, model: 'gpt-4')
...
Cosmos-LLM supports the following LLM providers:
...
export CLLM__GOOGLE__API_KEY=your-google-key
```
...
client = Cosmos::Llm.new(:openai)  # Uses configured API key
client = Cosmos::Llm.new(:anthropic, model: 'claude-3-sonnet-20240229')
```
...
**Returns:** `Cosmos::Llm::Client` instance
...
Cosmos-LLM includes a command-line tool (`cllm`) for quick interactions:

```bash
# One-shot completion
$ cllm prompt "What is Ruby?" -m gpt-3.5-turbo
...
```ruby
client = Cosmos::Llm.new(:openai, model: 'gpt-3.5-turbo')
...
content = Cosmos::Llm::ResponseHelpers.extract_content(response)

# Get token usage
tokens = Cosmos::Llm::ResponseHelpers.token_usage(response)
puts "Used #{tokens[:total_tokens]} tokens"

# Check why the response finished
reason = Cosmos::Llm::ResponseHelpers.finish_reason(response)
puts "Finished: #{reason}"

# Estimate cost
cost = Cosmos::Llm::ResponseHelpers.estimate_cost(response)
...
Cosmos::Llm::ProviderUtilities.supports_capability?(:openai, :streaming)
# => true

# Get all provider info
info = Cosmos::Llm::ProviderUtilities.all_provider_info
```
...
client = CLLM(:openai, model: 'gpt-4')
...
models = Cosmos::Llm.models(:openai)
puts models.inspect

# Or using provider utilities
models = Cosmos::Llm::ProviderUtilities.models_for_provider(:anthropic)
```

### Cloning Clients with Different Settings

```ruby
base_client = Cosmos::Llm.new(:openai, model: 'gpt-3.5-turbo')
...
loop do
  ...
  assistant_message = Cosmos::Llm::ResponseHelpers.extract_content(response)
...
results = providers.map do |provider, model|
  client = Cosmos::Llm.new(provider, model: model)
...
rescue Cosmos::Llm::RateLimitError => e
  puts "Rate limited, waiting..."
  sleep 5
  retry
rescue Cosmos::Llm::APIError => e
...
Thank you to the lite-llm and llm.datasette.io projects for their hard work, which was invaluable to this project. The cllm command line tool is patterned after the llm tool, though not as full-featured (yet).

The streaming jsonl code is from the ruby-openai repo; many thanks for their hard work.

## Contributing

Bug reports and pull requests are welcome on GitHub at https://github.com/cosmos-llm/cosmos-llm-ruby.

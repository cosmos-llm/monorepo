...
module Cosmos
  module Llm
    module Providers
      ...
      class DeepSeek < Cosmos::Llm::Providers::Base
        BASE_URL = 'https://api.deepseek.com'

        def default_api_key
          Cosmos::Llm.configuration.deepseek&.api_key || ENV['DEEPSEEK_API_KEY']
        end
        ...
        def handle_response(response, response_class = DeepSeekResponse)
          ...
          when 401 then raise Cosmos::Llm::AuthenticationError, parse_error_message(response)
          when 429 then raise Cosmos::Llm::RateLimitError, parse_error_message(response)
          when 400..499 then raise Cosmos::Llm::InvalidRequestError, parse_error_message(response)
          when 500..599 then raise Cosmos::Llm::ServerError, parse_error_message(response)
          else raise Cosmos::Llm::APIError, "Unexpected response code: #{response.status}"
          end
        end

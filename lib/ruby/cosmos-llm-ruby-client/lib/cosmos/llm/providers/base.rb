...
module Cosmos
  module Llm
    module Providers
      ...
      #   class MyProvider < Cosmos::Llm::Providers::Base
      #     def default_api_key
      #       Cosmos::Llm.configuration.my_provider&.api_key ||
      ...
      class Base
        ...
        #   provider = Cosmos::Llm::Providers::OpenAI.new(api_key: 'sk-...')
        # @example Initialize with default API key from configuration
        #   provider = Cosmos::Llm::Providers::OpenAI.new
        ...
        #   models = Cosmos::Llm::Providers::OpenAI.models
        #   # => ["gpt-4", "gpt-3.5-turbo", ...]
        def self.models
        ...
        # @raise [Cosmos::Llm::APIError] If the response indicates an API error
        # @raise [NotImplementedError] Subclasses must implement this method
        def handle_response(response)
        ...
        #
        # @raise [Cosmos::Llm::AuthenticationError] If API key is not configured
        # @return [void]
        ...
        def validate_api_key
          ...
          raise Cosmos::Llm::AuthenticationError,
                "API key required for #{provider_name}.\n\n" \
                "Set it using one of these methods:\n" \
                "  1. Environment variable: export #{env_var}=your-key\n" \
                "  2. Configuration block: Cosmos::Llm.configure { |c| c.#{provider_name.downcase}.api_key = 'your-key' }\n" \
                "  3. Client initialization: Cosmos::Llm.new(:#{provider_name.downcase}, api_key: 'your-key')"
        end

...
module Cosmos
  module Llm
    module Providers
      ...
      class AzureOpenai < Cosmos::Llm::Providers::Base
        BASE_URL_TEMPLATE = 'https://%s.openai.azure.com/openai/deployments/%s'

        def default_api_key
          begin
            Cosmos::Llm.configuration.azure_openai&.api_key
        ...
        def build_connection(base_url)
          Cosmos::Llm::HttpClient.new(url: base_url)
        end
        ...
        def raise_error(response)
          error_class = case response.status
                        when 401 then Cosmos::Llm::AuthenticationError
                        when 429 then Cosmos::Llm::RateLimitError
                        when 400..499 then Cosmos::Llm::InvalidRequestError
                        when 500..599 then Cosmos::Llm::ServerError
                        else Cosmos::Llm::APIError
                        end

          message = if error_class == Cosmos::Llm::APIError

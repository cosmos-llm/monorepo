...
module Cosmos
  module Llm
    ...
    class Client
      ...
      #   client = Cosmos::Llm::Client.new(:openai, model: 'gpt-4', api_key: 'sk-...')
      # @example Initialize with Anthropic provider
      #   client = Cosmos::Llm::Client.new(:anthropic, model: 'claude-3-opus-20240229')
      # @example Initialize without provider (use fluent interface)
      #   client = Cosmos::Llm::Client.new.with_provider(:openai).with_model('gpt-4')
      def initialize(provider_name = nil, options = {})
        unless options.is_a?(Hash)
          raise ArgumentError,
                "Options must be a Hash.\n" \
                "Example: Cosmos::Llm.new(:openai, model: 'gpt-4', api_key: 'sk-...')"
        end
        ...
        if provider_name && !provider_name.to_s.strip.empty?
          provider_class = Cosmos::Llm::Providers.provider_class_for(provider_name)
      ...
      #   client = Cosmos::Llm::Client.new(:openai)
      ...
      def embed(params = {})
        ...
        raise NotImplementedError,
              ...
              "Example: Cosmos::Llm.new(:openai).embed(model: 'text-embedding-ada-002', input: 'text')"
      end
      ...
      def with_provider(provider_name)
        if provider_name.nil? || provider_name.to_s.strip.empty?
          available = Cosmos::Llm::Providers.available_providers.join(', ')
          raise ArgumentError,

...
# The functions delegate to the main Cosmos::Llm module methods while providing shorter names.
...
# without requiring the full Cosmos::Llm module path. It's equivalent to calling
# Cosmos::Llm.new(provider, options).
#
...
# @return [Cosmos::Llm::Client] A new client instance
...
def LlmComplete(text, provider: :openai, model: nil, **options)
  Cosmos::Llm.complete(text, provider: provider, model: model, **options)
end
...
def LlmChat(messages, provider: :openai, model: nil, **options)
  Cosmos::Llm.chat(messages, provider: provider, model: model, **options)
end
...
def LlmModels(provider = :openai, **options)
  Cosmos::Llm.models(provider, **options)
end
...
# @yieldparam configuration [Cosmos::Llm::Configuration] The config object
# @return [void]

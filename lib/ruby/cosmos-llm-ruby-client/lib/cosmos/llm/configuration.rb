...
module Cosmos
  module Llm
    ...
    #   ENV['CLLM__OPENAI__API_KEY'] = 'sk-...'
    #   config = Cosmos::Llm::Configuration.new # Automatically loads from env
    #
    ...
    class Configuration
      ...
      def load_from_datasette
        ...
        begin
          ...
          Cosmos::Llm::Providers.providers.each do |provider|
      ...
      #   config = Cosmos::Llm::Configuration.new # Automatically loads these values
      def load_from_env
        ENV.each do |key, value|
          next unless key.start_with?('CLLM__')

          parts = key.split('__')
          next unless parts.length >= 3 # Must have CLLM__PROVIDER__SETTING

...
class TestHttpClient < Minitest::Test
  def setup
    ...
    @client = Cosmos::Llm::HttpClient.new(url: @base_url)
  end
  ...
  def test_response_wraps_faraday_response
    ...
    response = Cosmos::Llm::HttpClient::Response.new(faraday_response)
  ...
  def test_request_class_initialization
    ...
    request = Cosmos::Llm::HttpClient::Request.new(conn, '/path')

    assert_equal({}, request.headers)
    assert_nil request.body
    assert_instance_of Cosmos::Llm::HttpClient::RequestOptions, request.options
  end

  def test_stream_request_initialization
    conn = mock('connection')
    stream_request = Cosmos::Llm::HttpClient::StreamRequest.new(conn, '/path')
  ...
  def test_request_options_initialization
    conn = mock('connection')
    options = Cosmos::Llm::HttpClient::RequestOptions.new(conn)

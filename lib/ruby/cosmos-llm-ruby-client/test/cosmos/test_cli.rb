...
class TestCLI < Minitest::Test
  ...
  def test_prompt_command_basic
    ...
    Cosmos::Llm::Providers.expects(:model_id_to_provider).with('gpt-3.5-turbo').returns(Cosmos::Llm::Providers::OpenAI)
    Cosmos::Llm::Client.expects(:new).with(:openai).returns(client_mock)
  ...
  def test_prompt_command_with_model_option
    ...
    Cosmos::Llm::Providers.expects(:model_id_to_provider).with('gpt-4').returns(Cosmos::Llm::Providers::OpenAI)
    Cosmos::Llm::Client.expects(:new).with(:openai).returns(client_mock)
  ...
  def test_prompt_command_no_provider_found
    Cosmos::Llm::Providers.expects(:model_id_to_provider).with('unknown-model').returns(nil)
  ...
  def test_models_command
    ...
    Cosmos::Llm::Providers.expects(:providers).returns(%i[openai anthropic])

    Cosmos::Llm::Providers.expects(:provider_class_for).with(:openai).returns(openai_mock)
    Cosmos::Llm::Providers.expects(:provider_class_for).with(:anthropic).returns(anthropic_mock)
  ...
  def test_models_command_with_options_flag
    ...
    Cosmos::Llm::Providers.expects(:providers).returns([:openai])
    Cosmos::Llm::Providers.expects(:provider_class_for).with(:openai).returns(openai_mock)
  ...
  def test_conversations_command_no_conversations
    ...
    Dir.expects(:exist?).with(Cosmos::Llm::CLI::CONVERSATIONS_DIR).returns(true)
    Dir.expects(:glob).with("#{Cosmos::Llm::CLI::CONVERSATIONS_DIR}/*.json").returns([])
  ...
  def test_conversations_command_with_conversations
    ...
    Dir.expects(:exist?).with(Cosmos::Llm::CLI::CONVERSATIONS_DIR).returns(true)
    conversation_file = "#{Cosmos::Llm::CLI::CONVERSATIONS_DIR}/abc123.json"
    Dir.expects(:glob).with("#{Cosmos::Llm::CLI::CONVERSATIONS_DIR}/*.json").returns([conversation_file])
    ...
    File.expects(:exist?).with(Cosmos::Llm::CLI::LAST_CONVERSATION_FILE).returns(true)
    File.expects(:read).with(Cosmos::Llm::CLI::LAST_CONVERSATION_FILE).returns('abc123')
  ...
  def test_delete_conversation_command
    ...
    File.expects(:exist?).with(Cosmos::Llm::CLI::LAST_CONVERSATION_FILE).returns(true).twice
    File.expects(:read).with(Cosmos::Llm::CLI::LAST_CONVERSATION_FILE).returns('abc123')
    File.expects(:delete).with(Cosmos::Llm::CLI::LAST_CONVERSATION_FILE)

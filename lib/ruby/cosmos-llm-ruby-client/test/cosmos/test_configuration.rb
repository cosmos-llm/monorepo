...
class TestConfiguration < Minitest::Test
  ...
  def test_clear_reloads_from_env
    old_env = ENV['CLLM__ANTHROPIC__API_KEY']
    ENV['CLLM__ANTHROPIC__API_KEY'] = 'env_key'

    @config.clear

    assert_equal 'env_key', @config.anthropic.api_key

    ENV['CLLM__ANTHROPIC__API_KEY'] = old_env
  end

  def test_load_from_env_parses_dllm_variables
    old_env = {}
    %w[CLLM__OPENAI__API_KEY CLLM__ANTHROPIC__MODEL CLLM__GOOGLE__API_KEY].each do |key|
      old_env[key] = ENV[key]
    end

    ENV['CLLM__OPENAI__API_KEY'] = 'openai_key'
    ENV['CLLM__ANTHROPIC__MODEL'] = 'claude-3'
    ENV['CLLM__GOOGLE__API_KEY'] = 'google_key'
  ...
  def test_load_from_env_handles_malformed_variables
    old_env = ENV['CLLM__MALFORMED']
    ENV['CLLM__MALFORMED'] = 'should_be_ignored'
  ...
  def test_configuration_isolation
    config1 = Cosmos::Llm::Configuration.new
    config2 = Cosmos::Llm::Configuration.new

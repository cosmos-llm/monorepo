...
require 'ostruct'
require 'cosmos/llm/response_helpers'
...
class TestResponseHelpers < Minitest::Test
  ...
  def test_extract_content_from_valid_response
    content = Cosmos::Llm::ResponseHelpers.extract_content(@sample_response)
    assert_equal 'Hello, world!', content
  end

  def test_extract_content_from_nil_response
    content = Cosmos::Llm::ResponseHelpers.extract_content(nil)
    assert_nil content
  end
  ...
  def test_all_contents_from_multi_choice_response
    contents = Cosmos::Llm::ResponseHelpers.all_contents(@multi_choice_response)
  ...
  def test_all_contents_from_nil_response
    contents = Cosmos::Llm::ResponseHelpers.all_contents(nil)
  ...
  def test_all_contents_filters_invalid_choices
    ...
    contents = Cosmos::Llm::ResponseHelpers.all_contents(response)
    assert_equal 2, contents.length
    assert_equal ['Valid', 'Also valid'], contents
  end

  def test_token_usage_from_valid_response
    usage = Cosmos::Llm::ResponseHelpers.token_usage(@sample_response)
  ...
  def test_token_usage_from_nil_response
    usage = Cosmos::Llm::ResponseHelpers.token_usage(nil)
    assert_nil usage
  end

  def test_token_usage_from_response_without_usage
    response = OpenStruct.new(choices: [])
    usage = Cosmos::Llm::ResponseHelpers.token_usage(response)
    assert_nil usage
  end

  def test_finish_reason_from_valid_response
    reason = Cosmos::Llm::ResponseHelpers.finish_reason(@sample_response)
    assert_equal 'stop', reason
  end

  def test_finish_reason_from_truncated_response
    reason = Cosmos::Llm::ResponseHelpers.finish_reason(@truncated_response)
    assert_equal 'length', reason
  end

  def test_finish_reason_from_nil_response
    reason = Cosmos::Llm::ResponseHelpers.finish_reason(nil)
    assert_nil reason
  end
  ...
  def test_truncated_returns_true_for_length_finish_reason
    assert Cosmos::Llm::ResponseHelpers.truncated?(@truncated_response)
  end

  def test_truncated_returns_false_for_stop_finish_reason
    refute Cosmos::Llm::ResponseHelpers.truncated?(@sample_response)
  end

  def test_truncated_returns_false_for_nil_response
    refute Cosmos::Llm::ResponseHelpers.truncated?(nil)
  end

  def test_to_hash_returns_complete_hash
    hash = Cosmos::Llm::ResponseHelpers.to_hash(@sample_response)
  ...
  def test_to_hash_with_multi_choice_response
    hash = Cosmos::Llm::ResponseHelpers.to_hash(@multi_choice_response)
  ...
  def test_model_used_from_valid_response
    model = Cosmos::Llm::ResponseHelpers.model_used(@sample_response)
    assert_equal 'gpt-4', model
  end

  def test_model_used_from_nil_response
    model = Cosmos::Llm::ResponseHelpers.model_used(nil)
    assert_nil model
  end

  def test_model_used_from_response_without_model
    response = OpenStruct.new(choices: [])
    model = Cosmos::Llm::ResponseHelpers.model_used(response)
    assert_nil model
  end
  ...
  def test_estimate_cost_with_explicit_model_parameter
    ...
    cost = Cosmos::Llm::ResponseHelpers.estimate_cost(response, 'gpt-4')

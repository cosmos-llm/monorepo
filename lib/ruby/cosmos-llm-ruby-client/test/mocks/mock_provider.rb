...
module Cosmos
  module Llm
    module Providers
      class MockProvider < Cosmos::Llm::Providers::Base
        def default_api_key
          'mock_api_key'
        end
        ...
        def handle_response(response)
          ...
          when 401
            raise Cosmos::Llm::AuthenticationError, 'Mock authentication error'
          when 429
            raise Cosmos::Llm::RateLimitError, 'Mock rate limit error'
          when 400..499
            raise Cosmos::Llm::InvalidRequestError, 'Mock invalid request error'
          when 500..599
            raise Cosmos::Llm::ServerError, 'Mock server error'
          else
            raise Cosmos::Llm::APIError, "Mock unexpected response code: #{response.status}"
          end
        end
